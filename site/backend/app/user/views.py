...
from app.custom_functions import calc_file_hash, calc_string_hash, make_navigation, comment_sub, comments_to_list, get_profile_pic
from app.models import db, User, Video, Role, Comment, LikedBy, Profile_pic
from app.forms import FileuploadForm, AccountForm, CommentForm, VideoEditForm, SearchForm, ProfileImageForm
...
def settings():
    ...
    custom_content = {'title': 'Account Settings', 'active_page': '',
                      ...
                      'user_inf': [current_user.username, current_user.email,
                                   current_user.about_me, get_profile_pic(current_user),
    ...
    if request.method == 'POST':
        if proimg_form.submit_pro.data and proimg_form.validate():
            print("Shit went through")
            f = proimg_form.pro_pic.data
            if f is not None:
                ...
                f.save(filepath)
                npropic = Profile_pic(filepath=nfilename)
                current_user.profile_pic = npropic
                db.session.commit()
                custom_content['update'] = ['Congratulations, you updated your profile image', 'success']
        elif form.submit_acc.data and form.validate():
    ...
    return render_template('user_main.u_settings.html', logoinf=logoinf, right_bar=right_bar, form=form, proimg_form=proimg_form,
...
def play(fileh):
    ...
    if (vid is not None):
        ...
        video_info = {'deleted': False, 'pub_date' : vid.pub_date,
                          'owner' : owner.username, 'owner_pic' : get_profile_pic(current_user),
