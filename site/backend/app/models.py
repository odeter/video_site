...
from app import login_manager, rbac_manager, cus_error
...
class User(UserMixin, db.Model):
    ...
    profile_pic = db.relationship("Profile_pic", uselist=False, back_populates="owner")
    is_deleted = db.Column(db.Boolean, default=False)

    videos = db.relationship('Video', backref='users', lazy=True)
    u_reactions = db.relationship('LikedBy', backref='users', lazy=True)
    roles = db.relationship(
        'Role',
        secondary=users_roles,
        backref=db.backref('roles', lazy='dynamic')
    )
    u_comments = db.relationship("Comment", backref='users', lazy=True)

    # def add_propic(self, img):
    #     self.profile_pic.append(img)
...
class Profile_pic(db.Model):
    ...
    owner = db.relationship("User", back_populates="profile_pic")
...
class Video(db.Model):
    ...
    v_reactions = db.relationship('LikedBy', backref='video', cascade="all,delete", single_parent=True, lazy=True)
    vid_comments = db.relationship('Comment', cascade="all,delete", backref='video', lazy=True,
...
class Comment(db.Model):
    ...
    video1_id = db.Column(db.Integer, db.ForeignKey('video.id', ondelete='CASCADE'))
    ...
    responses = db.relationship("Comment", cascade="all,delete", single_parent=True,
                                backref=db.backref('comment', remote_side=[id]))
    c_reactions = db.relationship('LikedBy', cascade="all,delete", single_parent=True, backref='comment', lazy=True)
    ...
    def get_vid(self):
        img = (Video.query.filter_by(id=self.video1_id).first())
        return img

    def get_vid_img(self):
        img = (Video.query.filter_by(id=self.video1_id).first()).thumbpath
        return img

    def get_vid_of_id(self):
        vid_of = (Video.query.filter_by(id=self.video1_id).first()).vid_of_id
        return vid_of

    def add_reaction(self, reaction, user_id):
        reac_id = reaction.owner_id
        pre_like = LikedBy.query.filter_by(owner_id=user_id, comment_id=self.id).first()
        vid_del = (Video.query.filter_by(id=self.video1_id).first()).is_deleted
